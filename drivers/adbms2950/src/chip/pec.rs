//! Packet Error Code (PEC) calculations for the ADBMS2950B.

#![allow(dead_code)]

/// A calculated command PEC (CRC-15). See Table 30 on page 23 of the datasheet.
///
...
impl CommandPec {
    ...
    /// Command Packet Error Code Byte 0 (`PEC[14:7]`). See Table 30 on page 23 of the datasheet.
    pub const fn pec0(&self) -> u8 { (self.raw >> 8) as u8 }

    /// Command Packet Error Code Byte 1 (`PEC[7:0]`). Bit `0` will always be a trailing zero. See Table 30 on page 23 of the datasheet.
...
/// A calculated data PEC (CRC-10). See Table 31 (write) / Table 32 (read) on page 24 of the datasheet.
///
...
impl DataPec {
    ...
    ///
    /// See Table 31 of the datasheet. `PEC[9:8]` is stored in the first two bits of PEC0. The remaining 6 bits are zeroed on TX,
    /// or carry `CCNT[5:0]` on RX.
    pub const fn pec0(&self) -> u8 { (self.raw >> 8) as u8 }

    /// Data Packet Error Code Byte 1 (`PEC[7:0]`).
    ///
    /// See Table 31 of the datasheet.
    pub const fn pec1(&self) -> u8 { self.raw as u8 }
...
/// A data PEC for TX. This is something we calculate before transmitting. See Table 31 on page 24 of the datasheet.
#[rustfmt::skip]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
...
impl DataPecTx {
    ...
    /// Data Packet Error Code Byte 0 (`PEC[9:8]`).
    ///
    /// See Table 31 of the datasheet. `PEC[9:8]` is stored in the first two bits of PEC0.
...
/// A data PEC for RX. This is something we construct when we recieve a frame. See Table 32 on page 24 of the datasheet.
#[rustfmt::skip]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
...
impl DataPecRx {
    ...
    /// Data Packet Error Code Byte 0 (`PEC[9:8]`).
    ///
    /// See Table 32 of the datasheet. `PEC[9:8]` is stored in the first two bits of PEC0.
    /// This does not contain the `CCNT[5:0]` bits (see `ccnt()` for that).
    pub const fn pec0(&self) -> u8 { self.inner.pec0() & 0b00000011 } // get first 2 bits

    /// Returns the CCNT[5:0] data included in a `DataPecRx`. See Table 32 of the datasheet.
